...
const ProjectSection = ({ project }) => {
    ...
    return (
        <div className={"bg-primary flex flex-col gap-4 w-full h-profileSection max-h-profileSection  overflow-y-scroll rounded-md p-7 "}>
            ...
            <div className={"flex flex-col items-center gap-3 rounded-md justify-center w-full h-full"}>
                {project ? project.map((project, index) => (
                    ...
                    <Button className={"bg-button_border text-white"} size={"large"}>Projects / Discover the universe</Button>
                </div>}
            </div>
        </div>
    );
